...
/**
 * §16.3's three rulings, applied to one world. Returns the three numbers the
 * shader needs, in metres.
 *
 ...
 * physics.
 */
export function aerialParams(world, atmo = 1, hazeX = 1) {
  ...
  return {
    near: Math.min(70 * k, 1e9),
    far: Math.min(1700 * k, 1e9),
...
export function aerial(col, dist, V, sun, worldY,
  { near, far, hazeH, mistAmt = 1 } = REFERENCE_PARAMS, air = REFERENCE_AIR) {
  ...
  const pool = (1 - smoothstep(8, 46, worldY)) * smoothstep(120, 420, d0) * mistAmt;
...
export const AERIAL_GLSL = /* glsl */`
  ...
  vec4 aerial(vec3 col, float dist, vec3 V, vec3 sunDir, float worldY) {
    ...
    float pool = (1.0 - smoothstep(8.0, 46.0, worldY)) * smoothstep(120.0, 420.0, d0) * uAirMistAmt;
 ...
 */
export function aerialUniforms(world, atmo = 1, hazeX = 1, starT = 5778, elev = 13.5) {
  const p = aerialParams(world, atmo, hazeX);
