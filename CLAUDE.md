...
3. **Determinism.** Same seed + same code = same universe on every machine, forever. No `Math.random()`, no `Date.now()`, no un-seeded `performance.now()` in any generation path. `src/rng.js` is the only entropy source.
...
- **Determinism leaks.** Any `Math.random()`, wall-clock read, or iteration-order dependency in a generation path silently breaks shareable URLs.
